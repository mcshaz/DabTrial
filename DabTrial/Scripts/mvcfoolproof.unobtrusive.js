...
(function () {
    jQuery.validator.addMethod("is", function (value, element, params) {
        var $dependentProperty = $("#"+foolproof.getId(element, params["dependentproperty"])),
			...
			passOnNull = params["passonnull"],
			dependentValue;
		if ($(element).datepicker()) { 
			value = $(element).datepicker("getDate");
			if($dependentProperty.datepicker()){
				dependentValue = $dependentProperty.datepicker("getDate");
			} else if (isIsoDate($dependentProperty.val())){
				dependentValue=Date.Parse($dependentProperty.val());
			} else { //if not a standard date format, parse according to format used in the element being tested
				dependentValue=$.datepicker.parseDate($(element).datepicker("option","dateFormat"), $dependentProperty.val());
			}
		} else {
			dependentValue=$dependentProperty.val();
		}
	...
	{
		return /\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d/.test(input);
	}
